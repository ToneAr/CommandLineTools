...
cliInterfaceCommandsP =
KeyValuePattern[{(stringFormP :> expr_) ...}] | <||>;
...
parseUndefinedArgAndFlag[ optionList_, OptionsPattern[]] :=
	...
	Replace[StringSplit[Last[optionList],
	OptionValue["ArgumentDelimiter"]],
		...
		optionShortP,
			If[ OptionValue["POSIX"],
				...
				,
				StringDelete[StartOfString ~~ "-"][ First[optionList] ] -> Replace[StringSplit[Last[optionList], OptionValue["ArgumentDelimiter"]],
						{ a_ } :> a
					]
			]
		,
...
parseForDefinedArgs[flagList : {__String}, spec_, OptionsPattern[]] :=
	Block[{
				shortOption =
	First[flagList] // StringDelete[StartOfString ~~ "-"],
		...
		most = If[
			...
			List @@ #, {OrderlessPatternSequence[True, False]}] &) |
...
applyPositionalSpec[spec_, positionals_List] :=
	Module[{specAssoc = Association @ Cases[spec, Rule[k : positionalSpecKeyP, v_] :> (k -> v)]},
		MapIndexed[
			Function[{val, idx},
				With[{key = "$" <> ToString[idx[[1]]]},
					key -> With[{entry = Lookup[specAssoc, key, Missing[]]},
						Which[
							MissingQ[entry],
								val,
							...
							MatchQ[entry, Except[_Rule]],
								Replace[
									ResourceFunction["ToExpressionMatched"][val, entry],
									Except[entry] :> val
								],
							...
							MatchQ[entry, _Rule],
								Replace[
									ResourceFunction["ToExpressionMatched"][val, entry[[1]]],
									Except[entry[[1]]] :> val
								]
						]
					]
...
(* ::Subsubsection:: *)(* Main *)
CommandLineTools["Parse", form : stringFormP,
opts : OptionsPattern[]] := Module[{
			args = CommandLineTools["Parse", opts]
		},
	If[FailureQ[args],
			Return[args]
		];
	Replace[
		...
		"MessageTemplate" -> "Flag `` not found in the command line",
				...
				a : <| __Rule |> /;
	Length[DeleteDuplicates[Values[a]]] === 1 :>
					First[a]
			}
		]
];
CommandLineTools["Parse", opts : OptionsPattern[]] := Block[{
	...
	(*!Once[!*)
		Enclose[
			...
			parsed = Confirm @ parseFlagsAndArgs[
				flagsAndArgs,
			...
			ConfirmMatch[
				Join[
					...
					parsed,
					Association @ With[{spec = OptionValue["ArgumentSpecification"]},
						If[ spec =!= None,
							applyPositionalSpec[spec, allPositionals],
							MapIndexed[("$" <> ToString[#2[[1]]] -> #1) &, allPositionals]
						]
					]
				],
				argvP
...
OptionsPattern[]
	...
	Flatten[
			...
			Flatten @* ReplaceAll[
				{
						short : optionShortP :>
							StringInsert[ Rest @* Characters @ short, "-", 1 ]
					}
			],
			Identity
		],
...
Block[{ in, helpMessage, allCommands },
	Enclose[
			...
			helpMessage = CommandLineTools["Help",
			...
			While[! CommandLineTools`ShouldExit ,
				Pause[.01];
...
(* ::Subsection:: *)(* Log *)
CommandLineTools["Log", type_String, opts : OptionsPattern[]][
msg_String ] :=
...
Module[{info, patternLabel, fmtLine, longest},
	patternLabel[pat_] := Which[
			SameQ[pat, _Integer], "<Integer>",
			SameQ[pat, _Real],    "<Real>   ",
			SameQ[pat, _String],  "<String> ",
			SameQ[pat, _Symbol],  "<Symbol> ",
			SameQ[True | False] || SameQ[pat, _?BooleanQ], "<Boolean>",
			True, "<" <> ToString[pat, InputForm] <> ">"
		];
	info = Map[
			Function[e,
				Which[
					...
					MatchQ[e, {_String, _String}],
						<|
								...
								"Default" -> False
							|>,
					MatchQ[e, Rule[_String | {_String, _String}, Except[_Rule]]],
						<|
								"Long" -> If[ListQ[e[[1]]], e[[1, 1]], e[[1]]],
								"Short" -> If[ListQ[e[[1]]], e[[1, 2]], Missing[]],
								"Pattern" -> e[[2]],
								"Default" -> Missing[]
							|>,
					MatchQ[e, Rule[_String | {_String, _String}, Rule[_, _]]],
						<|
								"Long" -> If[ListQ[e[[1]]], e[[1, 1]], e[[1]]],
								"Short" -> If[ListQ[e[[1]]], e[[1, 2]], Missing[]],
								"Pattern" -> e[[2, 1]],
	...
	StringRepeat[" ", Max[0, longest + 8 - StringLength[flagPart]]];
			patTxt =
				If[MatchQ[pat, Except[_?BooleanQ]],
					patternLabel[pat]
				];
			defTxt = Which[
				MissingQ[def], "",
...
(* ::Subsubsection:: *)(* Main *)
CommandLineTools["Help", opts : OptionsPattern[]] := Module[{
			spec = OptionValue["ArgumentSpecification"],
	...
	If[TrueQ[OptionValue["InterfaceHelp"]] && Length[commands] > 0,
		result =
			result <> "\n\nCommands:\n\n" <>
			generateInterfaceCommandsHelp[commands]
	];
	(* Add options section if specification exists *)
	If[TrueQ[OptionValue["OptionHelp"]] && spec =!= None,
